...
#region {*************Metodos Importar Assortment*************}
		...
		public static function Val_Campos($rows, $limite, $nom_columnas, $cod_tempo, $depto) {
			...
			if ($val == TRUE) {
				...
				$tipoVal = 2;
				$dtciclo_vida = plan_compra::list_ciclo_vida();
				for ($i = 3; $i <= $limite; $i++) {
					$val2 = false;
			...
			if ($val == TRUE) {
				/*Validacion RNK VTA */
				$tipoVal = 3;
				$dtrnkventa = plan_compra::list_rnk();
				for ($i = 3; $i <= $limite; $i++) {
					$val2 = false;
			...
			if ($val == TRUE) {
				/*Validacion PIRAMIDE MIX */
				$tipoVal = 4;
				$dtpiramidemix = plan_compra::list_piramidemix();
				for ($i = 3; $i <= $limite; $i++) {
					$val2 = false;
			...
			if ($val == TRUE) {
				/*Validacion cluster */
				$tipoVal = 5;
				$dtcluster = plan_compra::list_cluster($cod_tempo, $depto);
				for ($i = 3; $i <= $limite; $i++) {
					$val2 = false;
					$key3 = 0;
			...
			if ($val == TRUE) {
				/*Validacion formato */
				$tipoVal = 6;
				$dtformato = plan_compra::list_Formato($cod_tempo, $depto);
				for ($i = 3; $i <= $limite; $i++) {
					$val2 = false;
			...
			if ($val == TRUE) {
				/*Validacion Ventana Debut */
				$tipoVal = 9;
				$ventanas = array("A", "B", "C", "D", "E", "F", "G", "H", "I");
				for ($i = 3; $i <= $limite; $i++) {
					$val2 = false;
			...
			if ($val == TRUE) {
				/*Validacion ID_CORPORTAIVO 10 LEN*/
				$tipoVal = 16;
				for ($i = 3; $i <= $limite; $i++) {
			...
			if ($val == TRUE) {
				...
				for ($i = 3; $i <= $limite; $i++) {
					if ($rows[$i][$nom_columnas['Codigo Marca']] <> null and
						...
						$key2 = false;
						foreach ($array1 as $val) {
							$var1Est = substr($val, 0, 1);
							$var1Clust = substr($val, 1, strlen($val));
				...
				if ($rows[$i][$nom_columnas['Linea']] != null and
					...
					foreach ($vent as $var) {
						...
						If ($rows[$i][$nom_columnas[$var]] > 0
							...
							if ($debut == "DEBUT" and strtoupper($rows[$i][$nom_columnas["Tipo de empaque"]]) == strtoupper("Curvado")) {
					...
					if ($val2[$nom_columnas['Linea']] != null and
						...
						foreach ($vent as $var) {
							...
							If ($val2[$nom_columnas[$var]] > 0
								...
								if (strtoupper($debut) == "DEBUT") {
									$ciclovida = $val2[$nom_columnas["Ciclo de Vida"]];
								}
								if (strtoupper($debut) == "DEBUT" and strtoupper($val2[$nom_columnas["Tipo de empaque"]]) == strtoupper("Curvado")) {
									$tipo_empaque = 'Curvado';
								}
...
#region {*************Metodos Importar bmt*************}
		...
		public static function Val_campos_bmt($rows, $limite, $nom_columnas, $depto, $temporada) {
			...
			if ($val == true) {
				$tipoVal = 3;
				$dt_grup = plan_compra::list_grupocompraBD($temporada, $depto);
				for ($i = 1; $i <= $limite; $i++) {
					$_exist = false;
			...
			if ($val == true) {
				$tipoVal = 6;
				$Colores = plan_compra::list_colores();
				for ($i = 1; $i <= $limite; $i++) {
					$_existe = false;
			...
			if ($val == true) {
				$tipoVal = 10;
				$VIA = array("MARITIMO", "AREA", "TERRESTRE");
				for ($i = 1; $i <= $limite; $i++) {
			...
			if ($val == true) {
				$tipoVal = 11;
				$Ciclo_vida = plan_compra::list_ciclo_vida();
				for ($i = 1; $i <= $limite; $i++) {
					$val2 = false;
