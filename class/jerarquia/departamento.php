<?php
	...
	class departamento extends \parametros {
		...
		public static function getDepartamentoBuscar($term) {
			$term = strtoupper($term);
			$login = strtoupper($_SESSION["login"]);
			$sql = "SELECT DISTINCT
						A.DEP_DEPTO
						, A.DEP_DESCRIPCION
						, A.DEP_DEPTO || ' - ' || A.DEP_DESCRIPCION AS DESCRIPCION
					FROM GST_MAEDEPTOS A
					WHERE
						(A.DEP_COC_DVS NOT IN ('G05','G08','G07','G10'))
						AND (A.DEP_DEPTO LIKE '%$term%' OR A.DEP_DESCRIPCION LIKE '%$term%')
						AND (EXISTS(SELECT DEP_DEPTO FROM PLC_ACCESO_DEPTO WHERE (DEP_DEPTO = A.DEP_DEPTO) AND (COD_USR = '$login') AND (ESTADO = 2)))
					ORDER BY
						A.DEP_DESCRIPCION";
