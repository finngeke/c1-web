    ...
    $.getJSON(url_busca_cod_tip_usr,function( data ) {
            ...
            $.getJSON(url_buscar_modulos_estados_desactivado,{ID_TIP_USR:tipo_usuario}, function (data_modulo) {
                ...
                $.getJSON(url_buscar_accion_estados_desactivado,{ID_TIP_USR:tipo_usuario}, function (data_accion) {
                    ...
                    //VALIDAR MODULO DE INVOICE INCOME
                    var verifica_modulo = $.grep(arreglo_modulos_desac,function(e){return e.id_modulo==21;});
                    if (verifica_modulo.length > 0){
                        $('#modulo_invoice_income').remove();
                    }
                    //VALIDAR BOTONES DEL MODULO DE INVOICE INCOME
                    var verifica_accion = $.grep(arreglo_accion_desac,function(e){return e.id_accion==62;});
                    if (verifica_accion.length > 0){
                        $("#btn_save_invoice_income").attr("disabled", "disabled");
